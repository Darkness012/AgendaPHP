var utils = {
  fixEvents(eventos){
    ...
    eventos.forEach(evento => {
      ...
      newEvent.title = evento.title;
      newEvent.allDay = evento.allDay==0?false:true;
      newEvent.start = evento.start;

      if(evento.allDay==0){
        newEvent.start = newEvent.start+" "+evento.start_hour;
        newEvent.end = evento.end+" "+evento.end_hour;
      }else{
        newEvent.end = utils.addDays(evento.start, 1);
      }

      eventsFixed.push(newEvent);
    });
...
class EventsManager {
    ...
    poblarCalendario(eventos) {
        let events = eventos?utils.fixEvents(eventos):[];
    ...
    actualizarEvento(evento) {
        ...
        form_data.append('start', start_date)
        if(!evento.allDay){
          form_data.append('end', end_date)
